...
cpdtr <- read.csv("coupon_detail_train.csv", header=T)
cpltr <- read.csv("coupon_list_train.csv", header=T)
cplte <- read.csv("coupon_list_test.csv", header=T)
ulist <- read.csv("user_list.csv", header=T)
cpvtr <- read.csv("coupon_visit_train.csv", header=T)# big file
...
train <- train[,c("COUPON_ID_hash","USER_ID_hash",
                  "GENRE_NAME","DISCOUNT_PRICE","PRICE_RATE","DISPPERIOD",
...
while (i2 < imax) {  # this loop takes a few minutes      
  ...
  trainv$DISCOUNT_PRICE <- 1;trainv$PRICE_RATE <- 1;train$DISPPERIOD <- 1;train$USABLE_DATE_sum <- 1;trainv[is.na(trainv)] <- 1
...
#Weight Matrix: GENRE_NAME, DISCOUNT_PRICE, PRICE_RATE, DISPPERIOD, large_area_name, small_area_name, VALIDPERIOD, USABLE_DATE_sum
require(Matrix)
weightm <- c(1.96, 1.25, -0.005, 1.25, 1.0, 4.50, 0.625, 0.4) # males weights
weightf <- c(1.75, 0.75, -0.005,1.50, 1.00, 4.50, 0.625, 0.25) # female weights

#Customize Weights
male_gen <- rep(weightm[1],13)
female_gen <- rep(weightf[1],13)

#Guys don't like Salons and massages
male_gen[c(2)] <- 1.8
male_gen[c(7)] <- .7
male_gen[c(10)] <- 2.10
female_gen[c(2)] <- 2.20
...
Wm <- as.matrix(Diagonal(x=c(male_gen, rep(weightm[2],1), rep(weightm[3],1), rep(weightm[4],1), rep(weightm[5],9), 
                             rep(weightm[6],55),rep(weightm[7],2),rep(weightm[8],1))))
Wf <- as.matrix(Diagonal(x=c(female_gen, rep(weightf[2],1), rep(weightf[3],1), rep(weightf[4],1), rep(weightf[5],9), 
                             rep(weightf[6],55),rep(weightf[7],2),rep(weightf[8],1))))

#calculation of cosine similarities of users and coupons
score = as.matrix(uchar[,2:84]) %*% Wm %*% t(as.matrix(test[,2:ncol(test)]))
score[ulist$SEX_ID=='f',] = as.matrix(uchar[ulist$SEX_ID=='f',2:84]) %*% Wf %*% t(as.matrix(test[,2:ncol(test)]))
...
write.csv(submission, file="tuning_test_MF2.csv", row.names=FALSE)
